...
button_to_key2 = {
   ...
   10:'R1', 9:'UP', 8:'L2',
   7:'DOWN', 6:'R2',
   5: 'LEFT', 4:'MODE', 3:'RIGHT',
   2:'None', 1:'None', 0:'None'
   }
###############################

'''PlayStation Remote Layout'''
button_layout = {
   'PAD': 13, 'PLAYSTATION': 12, 'R': 11,
   'L': 10, 'OPTIONS': 9, 'SHARE': 8,
   'R2': 7, 'L2': 6,
   'R1':5, 'L1': 4, 'TRIANGLE': 3,
   'CIRCLE': 2, 'X': 1, 'SQUARE':0
   }

button_to_key = {
   13:'PAD', 12:'PLAYSTATION', 11:'R',
   10:'L', 9:'OPTIONS', 8:'SHARE',
   7:'R2', 6:'L2',
   5: 'R1', 4:'L1', 3:'TRIANGLE',
   2:'CIRCLE', 1:'X', 0:'SQUARE'
   }
...
class Joystick():
    ...
    def set_button(self, button, key):
       value = button_layout[button]
       #self.nextKey = button_layout[button] = key
       #if self.nextKey != None:
       if value != None:
          #value = ord(self.nextKey)-97
          self.sameKey = button_to_key[value] = key
          return self.sameKey
    ...
    def mouse(self, win, mimg):
       ...
       while not self.isClosed():
          ...
          for event in pygame.event.get():
             ...
             mimg.move(axis_x, axis_y)
       mimg.undraw()
       
             
    def get_joy_key_mapping(self):
       """Returns a joystick key that always cooresponds with the same ascii lower case letter"""
       #Example Joybutton 1 is always 'a' and Joybutton4 is always 'd'
       for event in pygame.event.get():
           if event.type == pygame.JOYBUTTONDOWN:
              self.lastKey =  chr(int(event.dict['button'])+self.lc)
              JOYVAL = event.dict['button']
              self.dict[JOYVAL] = self.lastKey
              mergedict = {**self.dict, **new_key_map, **button_to_key}
              return mergedict[JOYVAL]

    def getKey(self):
        """Wait for user to press a key and return it as a string."""
        self.lastKey = self.get_joy_key_mapping()
        while self.lastKey == "":
            self.update()
            if self.isClosed(): raise GraphicsError("getKey in closed window")
            time.sleep(.1) # give up thread
        return self.get_joy_key_mapping()
    
    def checkKey(self):
        """Return last key pressed and interger associated with that key"""
